...
const resizeImage = async (blob: Blob, orientation: number = 1, maxSize: number = 2560, type: string = 'image/jpeg') => {
    ...
    try {
        ...
        if (needsNativeResize) {
            bitmap.close();
            try {
                // resizeQuality: 'high' uses GPU shaders on macOS and can crash on non-standard Canon JPEGs.
                // Using 'low' forces simple CPU scaling, which is incredibly reliable and prevents GPU crashes.
                bitmap = await createImageBitmap(blob, { resizeWidth: nativeResizeWidth, resizeQuality: 'low' });
            } catch (nativeResizeErr) {
                // If native resize STILL fails, fallback to full decode (riskier for memory, but our last hope)
