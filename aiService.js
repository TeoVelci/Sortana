...
var getAI = () => {
  const apiKey = typeof __GEMINI_API_KEY__ !== "undefined" && __GEMINI_API_KEY__ || import.meta.env.VITE_GEMINI_API_KEY || import.meta.env.VITE_API_KEY || import.meta.env.GEMINI_API_KEY || import.meta.env.API_KEY || atob("QUl6YVN5Q2lBVXI1cTVQQXJhU0VobXB3R2tGWEJSNXUzOHFpRmdv");
  if (!apiKey) {
...
var isValidImageBlob = async (blob) => {
  try {
...
`;
var workerBlob = new Blob([WORKER_SCRIPT], { type: "application/javascript" });
...
var runWorkerTask = (type, payload) => {
    ...
    while (this.highPriorityQueue.length > 0 || this.lowPriorityQueue.length > 0) {
      ...
      if (item) {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Request timed out")), 6e4));
...
var isValidImageForAnalysis = (file) => {
...
var analyzeVideoMetadata = async (rawMetadata) => {
  try {
    const ai = getAI();
...
var getFriendlyCameraName = (make, model) => {
  ...
  const isSony = brand.toLowerCase().includes("sony") || name.startsWith("ILCE-") || name.startsWith("FX") || name.startsWith("ZV-") || name.startsWith("A7") || name.startsWith("A9") || name.startsWith("A1") || name.includes("XAVC") || name.includes("XDCAM");
  if (isSony) {
...
var extractDetailedMetadata = async (file) => {
  try {
...
var processFileForDisplay = async (file, maxSize = 2560) => {
  let sourceBlob = file;
  let orientation = 1;
  try {
...
var prepareImageForAI = async (file, preProcessedBlob) => {
  let sourceBlob = preProcessedBlob || file;
    ...
    if (extracted) {
      ...
      try {
        ...
        for (let i = 0; i < bytes.length - 4; i++) {
          if (bytes[i] === 255 && bytes[i + 1] === 216 && bytes[i + 2] === 255) {
  ...
  try {
    const smallBlob = await runWorkerTask("resizeImage", {
      file: sourceBlob,
      orientation: 1,
      // AI usually handles orientation, but we normalize anyway
      maxSize: 1024,
      type: "image/jpeg"
    });
...
var extractVideoFrames = async (file) => {
  return new Promise((resolve, reject) => {
...
var callAIWithRetry = async (apiCall, retries = 3, priority = "low") => {
  for (let i = 0; i < retries + 1; i++) {
    try {
      ...
      if (isServerOverloaded && i < retries) {
        await new Promise((resolve) => setTimeout(resolve, 2e3 * Math.pow(2, i)));
        continue;
      }
      throw error;
    }
...
var generateTagsForBatch = async (batch) => {
  try {
    const apiKey = typeof __GEMINI_API_KEY__ !== "undefined" && __GEMINI_API_KEY__ || import.meta.env.VITE_GEMINI_API_KEY || import.meta.env.VITE_API_KEY || import.meta.env.GEMINI_API_KEY || import.meta.env.API_KEY;
    if (!apiKey) throw new Error("API Key missing");
    ...
    const processedImages = await Promise.all(batch.map(async (item) => {
      try {
        ...
        console.warn(`Upload attempt failed for ${item.file.name || "blob"}:`, e);
      ...
      if (Array.isArray(results)) {
        return results.map((r, index) => {
          ...
          return {
            ...
            tags: Array.isArray(r?.tags) ? r.tags.filter((t) => t != null).map(String) : [],
...
var proposeOrganization = async (files, strategy = "smart_event") => {
  const apiKey = typeof process !== "undefined" && process.env && process.env.GEMINI_API_KEY || typeof process !== "undefined" && process.env && process.env.API_KEY || import.meta.env.VITE_GEMINI_API_KEY || import.meta.env.VITE_API_KEY || import.meta.env.GEMINI_API_KEY || import.meta.env.API_KEY;
  if (!apiKey) throw new Error("API Key missing");
  ...
  const fileList = files.map((f) => ({
    ...
    // Include time for accurate event clustering
    type: f.type,
    make: f.make || "Unknown",
    // Explicit fallback for AI context
...
var setFiltersTool = { name: "set_filters", description: "Filter files.", parameters: { type: Type.OBJECT, properties: { rating: { type: Type.NUMBER }, flag: { type: Type.STRING }, search: { type: Type.STRING } } } };
var createFolderTool = { name: "create_folder", description: "Create folder.", parameters: { type: Type.OBJECT, properties: { name: { type: Type.STRING } }, required: ["name"] } };
var listProjectsTool = { name: "list_projects", description: "List projects.", parameters: { type: Type.OBJECT, properties: { refresh: { type: Type.BOOLEAN } } } };
var navigateTool = { name: "navigate", description: "Navigate.", parameters: { type: Type.OBJECT, properties: { page: { type: Type.STRING } }, required: ["page"] } };
var getStatsTool = { name: "get_storage_stats", description: "Get stats.", parameters: { type: Type.OBJECT, properties: { unit: { type: Type.STRING } } } };
var copilotTools = [setFiltersTool, createFolderTool, listProjectsTool, navigateTool, getStatsTool];
var initializeCopilotChat = () => {
  const ai = getAI();
...
export {
  ...
  isValidImageForAnalysis,
  prepareImageForAI,
  processFileForDisplay,
  proposeOrganization
};
